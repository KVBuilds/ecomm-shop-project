...
const Steps = () => {
    ...
    return ( <ol className="rounded-md bg-white lg:flex lg:rounded-none lg:border-1 lg:border-r lg:border-gray-200">
        {STEPS.map((step, i) => {
        const isCurrent = pathname.endsWith(step.url) //Checks for any array element if the condition is applied.
        ...
        const imgPath = `/snake-${i + 1}.png`
            ...
            return <li key={step.name} className="relative overflow-hidden lg:flex-1">
                ...
                <span className={cn(i !== 0 ? 'lg:pl-9' : '', "flex items-center px-6 py- text-sm font-medium")}>
                    <span className="flex-shrink-0">
                        <img src={imgPath} className={cn("flex h-20 w-20 object-contain items-center justfiy-center", {

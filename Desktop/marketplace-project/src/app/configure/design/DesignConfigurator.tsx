...
const DesignConfigurator = ({
    ...
    const {mutate: saveConfig} = useMutation({
        mutationKey: ["save-config"], // important for cache and invalidation
    ...
    const [options, setOptions] = useState<{
     ...
     // helps define the positon for saving and keeping track - dimension and position
    ...
    async function saveConfiguration() {
        try {
            ...
            //Converts and then exports image
            ...
            //Passes config Id from core.ts file and into the database
                        ...
                        <div className="relative mt-4 h-full flex flex-col justify-between">
                                        ...
                                        {COLORS.map((color) => (
                                            <RadioGroup.Option 
                                                ...
                                                className={({ active, checked }) => 
                                                    cn("relative -m-0.5 flex cursor-pointer items-center justify-center rounded-full p-0.5 active:ring-0 active:outline-none focus:outline-none border-2 border-transparent",
                                                        {
                            ...
                            <div className="flex flex-col gap-3 w-full">
                                ...
                                {[MATERIALS, FINISHES].map(({name, options: selectableOptions}) => (
                                    <RadioGroup key={name} value={options[name as keyof typeof options]}
                                    onChange={(val) => {
                                        setOptions((prev) => ({
                                            ...prev,
                                            //It can be both material or finish.
                                            [name as keyof typeof options]: val,
                                        }))
                                    }}>
                                        <Label>
                ...
                <div className="w-full px-8 h-16 bg-white">
                    ...
                    <div className="w-full h-full flex justify-end items-center">
                        <div className="w-full flex gap-6 items-center">
                            ...
                            <Button onClick={() => saveConfig({configId, color: options.color.value, finish: options.finish.value, material: options.material.value, model: options.model.value})} size="sm" className="w-full">Continue
